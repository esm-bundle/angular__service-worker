...
const packageJson = JSON.parse(
  fs
    .readFileSync(
      path.resolve(
        __dirname,
        "node_modules/@angular/service-worker/package.json"
      )
    )
    .toString()
);
...
const packages = ["2015", "2020"]
  .map((ecma) => [
    {
      ecma,
      angularPackage: "@angular/service-worker",
      filename: "service-worker",
    },
    {
      ecma,
      angularPackage: "@angular/service-worker/config",
      filename: "config",
    },
  ])
  .flat();
...
function createConfig({ ecma, prod, format, angularPackage, filename }) {
  ...
  return {
    input: path.join(
      __dirname,
      `node_modules/@angular/service-worker/fesm${ecma}/${filename}.mjs`
    ),
    output: {
